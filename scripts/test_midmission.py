...
from workers import jobs, emails, missions 
...
def send_mid_mission_notif(db, config, env):
    global _utils
    ...
    for _phase in phases:
        ...
        if mission is None:
            ...
            mission = makeMission(db, config, _phase, False)
        ...
        _job = mongoapi.make_job('warn.mission_in_progress', payload, mongoapi.tool_user['_id'])
        
        # do it 
        print(_job)
        if True:
            #ok, _job2 = mission_reports.create_mission_report(_job, db, config)
            ok, _job2 = missions.warn_mission_in_progress(_job, db, config)
...
if __name__ == '__main__':
    ...
    send_mid_mission_notif(DB, CONF, env)
'''